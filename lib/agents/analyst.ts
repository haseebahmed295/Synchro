...
export const RequirementSchema = z.object({
  id: z.string().regex(/^REQ_[A-Z0-9]+$/, 'ID must follow pattern REQ_[A-Z0-9]+'),
...
export class AnalystAgent {
   ...
   */
  async ingestText(rawText: string, projectId: string, userId: string): Promise<Requirement[]> {
...
Return a JSON object with an array of requirements.`
    ...
    try {
      ...
      throw new Error(`Requirement ingestion failed: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
...
Return an array of JSON Patch operations. Only include operations for fields that need to change.`
    ...
    try {
      ...
      throw new Error(`Surgical update failed: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
   ...
   */
  generateRequirementId(existingIds: Set<string>, prefix: string = 'REQ'): string {
    let id: string
    let attempts = 0
    ...
    do {
      ...
      if (attempts >= maxAttempts) {
        throw new Error('Failed to generate unique requirement ID after maximum attempts')
      }
