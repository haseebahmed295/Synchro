...
describe('JudgeAgent', () => {
  ...
  describe('validateDiagramConsistency', () => {
    ...
    it('should detect circular inheritance', async () => {
      ...
      const cycleIssue = issues.find(i => i.message.includes('circular inheritance'))
  ...
  describe('validateWithRefinement (Critic/Refine loop)', () => {
    ...
    it('should escalate after max refinement iterations', async () => {
      ...
      expect(result.reasoningLog).toContain('Validation failed after 2 refinement iterations')
      expect(result.reasoningLog.some(log => log.includes('Escalating to user'))).toBe(true)
    })
    ...
    it('should handle refiner failure', async () => {
      ...
      const mockRefiner = vi.fn().mockRejectedValue(new Error('Refinement failed'))
      ...
      expect(result.requiresEscalation).toBe(true)
      expect(result.reasoningLog.some(log => log.includes('Refinement failed'))).toBe(true)
    })
    ...
    it('should track iteration count correctly', async () => {
      ...
      const mockValidator = vi.fn()
        .mockResolvedValueOnce([{ severity: 'error', message: 'Error 1', affectedArtifacts: [] }])
        .mockResolvedValueOnce([{ severity: 'error', message: 'Error 2', affectedArtifacts: [] }])
        .mockResolvedValueOnce([{ severity: 'error', message: 'Error 3', affectedArtifacts: [] }])
  ...
  describe('validateArchitectOutput', () => {
    it('should validate and refine Architect diagram output', async () => {
      ...
      const result = await agent.validateArchitectOutput(diagram, mockRefineCallback)

      expect(result.requiresEscalation).toBe(false)
      expect(result.output).toEqual(refinedDiagram) // Changed from result.diagram to result.output
  ...
  describe('generateRefinementPrompt', () => {
    it('should generate clear refinement prompt', () => {
      const feedback: RefinementFeedback = {
        ...
        suggestions: ['Remove one inheritance edge', 'Add connections or remove node'],
        iterationCount: 1,
      }
