...
import { AnalystAgent, RequirementSchema, type Requirement } from '../analyst'
...
describe('AnalystAgent', () => {
  ...
  describe('ingestText', () => {
    it('should extract requirements from text', async () => {
      ...
      const result = await analyst.ingestText(rawText, mockProjectId, mockUserId)
...
• All responses must be in JSON format
  ...
  describe('surgicalUpdate', () => {
    it('should generate JSON patches for requirement updates', async () => {
      ...
      const delta = 'Change priority to high and update title to "Updated Title"'
      const result = await analyst.surgicalUpdate(existingReq, delta, mockUserId)
  ...
  describe('Stable Key Preservation Property', () => {
    it('should never modify the id field in patches', async () => {
      ...
      const scenarios = [
        {
          ...
          patches: [{ op: 'replace' as const, path: '/priority', value: 'high' }],
        },
        {
          delta: 'Update description',
          patches: [{ op: 'replace' as const, path: '/description', value: 'New description' }],
        },
        {
          delta: 'Add a tag',
          patches: [{ op: 'add' as const, path: '/metadata/tags/-', value: 'security' }],
        },
      ]
      ...
      for (const scenario of scenarios) {
        ...
        const result = await analyst.surgicalUpdate(existingReq, scenario.delta, mockUserId)
